...
void main() {
  ...
  setUp(() async {
    supportDir = await Directory.systemTemp.createTemp(
      'elibrary_db_migration_support_',
    );
    documentsDir = await Directory.systemTemp.createTemp(
      'elibrary_db_migration_documents_',
    );
    libraryRootDir = await Directory.systemTemp.createTemp(
      'elibrary_db_migration_root_',
    );
    ...
    final databaseDirectory = Directory(
      '${libraryRootDir.path}${Platform.pathSeparator}Databases',
    );
    await databaseDirectory.create(recursive: true);
    final emptyELibraryDb = await databaseFactory.openDatabase(
      '${databaseDirectory.path}${Platform.pathSeparator}eLibrary.db',
    );
    await ELibrarySchema.ensure(emptyELibraryDb);
    await emptyELibraryDb.close();
  ...
  test('copy is idempotent and preserves existing eLibrary rows', () async {
    ...
    expect(
      firstReport.tableReports
          .firstWhere((r) => r.tableName == 'library_items')
          .wouldInsertRowCount,
      0,
    );
    expect(
      firstReport.tableReports
          .firstWhere((r) => r.tableName == 'elibrary_install_estimates')
          .wouldInsertRowCount,
      0,
    );
