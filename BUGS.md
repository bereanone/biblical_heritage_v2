...
* **A raw `href` join between the two tables does not work, even when both exist for the same item.** `library_document_blocks.source_href` is a real, normalized EPUB spine path (`library_document_canonicalizer.dart:1173-1181`). But the one import path that writes both tables for the same `library_item_id` (`sourceType == 'pioneer_egw_epub_source'`, via `pioneer_text_import_service.dart`) gives its `library_navigation_items` rows a *synthetic* href (`captured/{workAbbrev}/chapter_01_{slug}.html`, built at `pioneer_text_import_service.dart:5091-5109`) that has no relationship to the real spine path. Only normalized-label matching is viable as a join key.
* **Most canonical items have zero `library_navigation_items` rows at all.** Anything acquired as a plain `official_download` (the primary, first-class acquisition path — `library_acquisition_batch_runner.dart:90`) goes straight EPUB → canonicalizer → `library_document_blocks`, and `library_catalog_service.dart` only ever *reads* `library_navigation_items`, never writes it. Only the `pioneer_egw_epub_source` lineage populates both tables. So any fix must degrade to today's flat-but-nested behavior by default, not assume nav rows exist.
* **Recommended approach:** in `canonical_library_reader.dart`'s `_openContents`, read-only and additive — query `library_navigation_items` for the same `library_item_id`, build its tree via the existing `buildLibraryNavigationTree()` (`lib/features/library/presentation/library_navigation_tree.dart:16`, already handles the parent/child logic — reuse, don't reimplement), and match `heading_role: 'chapter'` blocks to nav items by normalized label text (reusing whatever normalization `library_section_heuristics.dart` already has, plus the existing `_isNavigationSupportEntry` filter from `library_contents_popup.dart:168-179` to skip nav rows that shouldn't be matchable sections). No nav rows for the item → render exactly as today, no behavior change. No schema change, no data duplication.
* Alternatives considered and set aside: writing a derived `section_parent_href` onto `library_document_blocks` at canonicalization time (rejected — the canonicalizer has no visibility into the conceptual TOC hierarchy, that structure only exists in the nav pipeline, which frequently hasn't even run for these items — and it's exactly the data duplication the fix is meant to avoid); deriving grouping purely from spine order/heuristics with no nav dependency (viable as a later enhancement for the common `official_download`-only case, but duplicates logic that's already correct in the nav-item indexing pipeline rather than reusing it, and spine-adjacency isn't the same thing as conceptual nesting).
* Not yet implemented — proposed for review before writing any code, per your steer on priority.
